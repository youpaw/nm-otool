...
static t_mach_header_32	*get_header(void *map_start, size_t bin_size, \
	t_check_sizes *check)
{
	t_mach_header_32 *header;
	check->full = sizeof(t_mach_header_32);
	if (bin_size < check->full)
		return (NULL);
	header = (t_mach_header_32 *)map_start;
	check->cmds = 0;
	return (header);
}
