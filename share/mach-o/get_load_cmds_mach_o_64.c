...
static struct mach_header_64	*get_header(void *map_start, size_t bin_size, \
	t_check_sizes *check)
{
	struct mach_header_64	*header;

	check->full = sizeof(struct mach_header_64);
	if (bin_size < check->full)
		return (NULL);
	header = (struct mach_header_64 *)map_start;
	check->cmds = 0;
	return (header);
}
	...
	t_check_sizes *check)
{
	...
	if (lc->cmd == LC_SEGMENT_64)
	{
		...
		if (bin_size < (seg_cmd->fileoff + seg_cmd->filesize))
			return (1);
		if (seg_cmd->cmdsize < (sizeof(struct segment_command_64) + \
			seg_cmd->nsects * sizeof(struct section_64)))
			return (1);
	}
