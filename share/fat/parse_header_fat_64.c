...
static int	get_arch(struct fat_arch_64 *arch, t_binary_info *bin_info, \
	size_t mapoff)
{
	struct fat_arch_64	*arch_ptr;

	if (bin_info->size < mapoff + sizeof(struct fat_arch_64))
		return (1);
	arch_ptr = (struct fat_arch_64 *)((char *)bin_info->mapstart + mapoff);
	if (bin_info->swap)
	{
		arch->cputype = ft_swap_int32(arch_ptr->cputype);
		arch->cpusubtype = ft_swap_int32(arch_ptr->cpusubtype);
		arch->offset = ft_swap_uint64(arch_ptr->offset);
		arch->size = ft_swap_uint64(arch_ptr->size);
	}
	else
	{
...
static int	find_arch(struct fat_arch_64 *arch, struct fat_header *header, \
	t_binary_info *bin_info)
{
	...
	while (narch < header->nfat_arch)
	{
		...
		if (arch->cputype == CPU_TYPE_X86_64 && \
			(arch->cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_X86_64_ALL)
			return (0);
