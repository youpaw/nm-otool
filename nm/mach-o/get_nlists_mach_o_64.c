...
static void	get_nlist(struct nlist_64 *nlist, size_t mapoff, \
	t_binary_info *bin_info)
{
	struct nlist_64	*nl_ptr;

	nl_ptr = (struct nlist_64 *)((char *)bin_info->mapstart + mapoff);
	nlist->n_type = nl_ptr->n_type;
	nlist->n_sect = nl_ptr->n_sect;
	if (bin_info->swap)
	{
		nlist->n_un.n_strx = ft_swap_uint32(nl_ptr->n_un.n_strx);
		nlist->n_desc = ft_swap_uint16(nl_ptr->n_desc);
		nlist->n_value = ft_swap_uint64(nl_ptr->n_value);
	}
	else
	{
...
t_vec	*get_nlists_mach_o_64(struct symtab_command *symtab_cmd, \
	t_binary_info *binary_info)
{
	...
	if ((binary_info->archsize + binary_info->archoff) < \
		(symtab_cmd->symoff + (symtab_cmd->nsyms * sizeof(struct nlist_64))))
		print_nt_error(E_NT_TRMLF);
	else
	{
		mapoff = symtab_cmd->symoff;
		nlists = ft_vec_new(NLISTS_VEC_CAPACITY, sizeof(struct nlist_64), NULL);
		cnt = 0;
		while (cnt < symtab_cmd->nsyms)
		{
			get_nlist(&nlist, mapoff, binary_info);
			ft_vec_push(nlists, &nlist);
			mapoff += sizeof(struct nlist_64);
			cnt++;
		}
	}
	return (nlists);
