...
static int get_sects(t_segment_cmd_32 *seg, size_t seg_size, t_vec *sects)
{
	size_t			cnt;
	t_section_32	*sect;
	size_t			sect_size;
	...
	sect = (t_section_32 *) ((char *) seg + seg_size);
...
t_vec		*get_sections_mach_o_32(t_vec *load_cmds)
{
	...
	seg_size = sizeof(t_segment_cmd_32);
	cnt = 0;
	while (cnt < load_cmds->size)
	{
		...
		{
			if (get_sects((t_segment_cmd_32 *) lc, seg_size, sects))
			{
				ft_vec_del(&sects);
				break;
			}
